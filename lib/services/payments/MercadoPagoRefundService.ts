...
export class MercadoPagoRefundService implements IRefundService {
  ...
  constructor(accessToken?: string) {
    ...
    console.log(
      `[MercadoPagoRefundService] Inicializado${
        accessToken ? ' con credenciales del tenant' : ' con credenciales globales'
      }`
    );
  }
   ...
   */
  private checkRefundStatus(mpPayment: any): {
    canRefund: boolean;
   ...
   */
  private validateRefundTimeLimit(mpPayment: any): {
    valid: boolean;
    ...
    if (!mpPayment.date_approved) {
      return {
        valid: false,
        daysSinceApproval: 0,
        error: 'Fecha de aprobación no disponible',
      };
    }
  ...
  async processRefund(params: ProcessRefundParams): Promise<RefundResult> {
    try {
      ...
      // Montos en ARS (pesos). `params.amount` ya está en pesos.
      const totalPaid = mpPayment.transaction_amount || 0;
      const refundAmount = params.amount ? params.amount : undefined;
      ...
      const refundResponse = await refund.create({
        ...
        body: refundAmount !== undefined ? { amount: refundAmount } : {},
        requestOptions: {
          idempotencyKey,
        },
      });
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          msg.includes('ya fue reembolsado') ||
          msg.includes('refunded')
        ) {
          return { success: false, error: 'El pago ya fue reembolsado previamente' };
        }
        ...
        if (
          ...
          return {
            success: false,
            error:
              'Saldo insuficiente en la cuenta de Mercado Pago para procesar el reembolso. Por favor, verifica tu saldo disponible.',
          };
        }

        if (msg.includes('expired') || msg.includes('expirado') || msg.includes('180')) {
          return {
            success: false,
            error:
              'El plazo para reembolsar este pago ha expirado (máximo 180 días desde la aprobación)',
          };
        }
      ...
      return { success: false, error: 'Error desconocido al procesar el reembolso' };
    }
  }
}
