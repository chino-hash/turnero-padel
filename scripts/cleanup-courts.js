...
async function cleanupDuplicateCourts() {
  ...
  try {
    ...
    for (const [label, list] of groups.entries()) {
      ...
      list.forEach(c => console.log(`   - ${c.id} | ${c.name} | activa=${c.isActive}`))

      const exactMatch = list.find(c => (c.name || '').toLowerCase() === label)
      const activeExact = exactMatch && exactMatch.isActive ? exactMatch : null
      const byCreatedDesc = [...list].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      const withValidHours = byCreatedDesc.find(c => safeParseOperatingHours(c.operatingHours))
      ...
      for (const c of list) {
        ...
        if (!c.isActive) {
          console.log(`   ↪️ Ya inactiva: ${c.id} | ${c.name}`)
          continue
        }
    ...
    console.log('\n📋 Resumen:')
    if (changes.length === 0) console.log('   No hubo cambios; sin duplicados activos.')
    else changes.forEach(ch => console.log(`   - ${ch.action} -> ${ch.id} | ${ch.name}`))

    const activeCourts = await prisma.court.findMany({ where: { isActive: true }, select: { id: true, name: true } })
    console.log(`\n✅ Activas (${activeCourts.length}):`)
    activeCourts.forEach(c => console.log(`   - ${c.id} | ${c.name}`))
  } catch (error) {
