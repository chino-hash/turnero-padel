...
import { BookingConflictError } from '../errors/BookingErrors';
...
export class BookingService {
  ...
  return {
      ...
      expiresAt: (booking as any).expiresAt ? (booking as any).expiresAt.toISOString() : null,
  ...
  // Obtener minutos de expiración desde SystemSetting
  private async _getExpirationMinutes(): Promise<number> {
    try {
      const setting = await prisma.systemSetting.findUnique({
        where: { key: 'booking_payment_expiration_minutes' }
      });
      ...
      console.error('Error obteniendo minutos de expiración:', error);
  ...
  async createBooking(input: CreateBookingInput, userId: string): Promise<ApiResponse<BookingWithDetails>> {
    try {
      ...
      // Validar que los campos de precio existan
      if (!court.basePrice || court.basePrice <= 0) {
        console.error('Court basePrice is invalid:', court.basePrice);
        return {
          success: false,
          message: 'Error de configuración: la cancha no tiene un precio base válido',
          error: 'Error de configuración de la cancha'
        };
      }

      const priceMultiplier = court.priceMultiplier || 1.0;
      const totalPrice = Math.round(court.basePrice * priceMultiplier * (durationMinutes / 60));
      ...
      // Obtener minutos de expiración y crear reserva (la verificación de disponibilidad se hace dentro de la transacción)
      const expirationMinutes = await this._getExpirationMinutes();
      const booking = await this.repository.create(createData, expirationMinutes);
      ...
      // Manejar BookingConflictError específicamente
  ...
  async createPaymentPreference(bookingId: string): Promise<ApiResponse<{ preferenceId: string; initPoint: string; sandboxInitPoint?: string }>> {
    try {
      // Obtener la reserva
      ...
      const preference = await paymentProvider.createPreference({
        ...
        amount: booking.depositAmount, // O totalPrice según la lógica de negocio
        expiresAt: booking.expiresAt,
        userId: booking.userId,
        backUrls: {
          success: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/reservas/exito`,
          failure: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/reservas/error`,
          pending: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/reservas/pendiente`
        }
      });
  ...
  async cancelBooking(
    ...
    try {
      ...
      // Si tiene pagos y aplica reembolso, procesarlo
      // Importar dinámicamente para evitar dependencias circulares
      if (existingBooking.paymentStatus !== 'PENDING') {
        try {
          const { processRefundForCancellation } = await import('./payments/RefundService');
          const refundResult = await processRefundForCancellation(
            id, 
            cancellationReason || 'Cancelado por usuario'
          );
          
          if (!refundResult.success && !refundResult.error?.includes('No aplica')) {
            // Error crítico - podría requerir intervención manual
            console.error('Error procesando reembolso:', refundResult.error);
            // TODO: Notificar al admin
          } else if (refundResult.success) {
            console.log(`Reembolso procesado exitosamente para reserva ${id}: ${refundResult.refundId}`);
          }
        } catch (refundError) {
          // Continuar con la cancelación aunque falle el reembolso
          console.error('Error procesando reembolso (continuando con cancelación):', refundError);
        }
      }
