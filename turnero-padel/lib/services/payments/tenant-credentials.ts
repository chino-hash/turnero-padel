...
import { decryptCredential } from '../../encryption/credential-encryption';
...
function tryDecrypt(encrypted: string | null): string | null {
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (errorMessage.includes('credential_encryption_key') || 
      ...
      if (errorMessage.includes('bad decrypt') || 
        ...
        if (!encrypted.includes(':') || encrypted.split(':').length !== 3) {
          console.warn(`[TenantCredentials] Credencial no está encriptada (formato antiguo), usando directamente`);
          return encrypted;
        }
      }
    }
...
export async function getTenantMercadoPagoCredentials(
  ...
  if (cached && Date.now() < cached.expiresAt) {
    console.log(`[TenantCredentials] Usando credenciales del cache para tenant ${tenantId}`);
  ...
  if (!tenant.isActive) {
    throw new Error(`Tenant ${tenantId} está inactivo. No se pueden usar credenciales de Mercado Pago.`);
  }
  ...
  try {
    accessToken = tryDecrypt(tenant.mercadoPagoAccessToken) || tenant.mercadoPagoAccessToken;
    
    if (tenant.mercadoPagoWebhookSecret) {
      webhookSecret = tryDecrypt(tenant.mercadoPagoWebhookSecret);
    }
  } catch (error) {
    console.error(`[TenantCredentials] Error desencriptando credenciales para tenant ${tenantId}:`, error);
    throw new Error(
      `Error al desencriptar credenciales del tenant ${tenantId}: ${error instanceof Error ? error.message : 'Error desconocido'}`
    );
  }

  // Validar formato básico del accessToken
  if (!accessToken.startsWith('APP_USR-') && !accessToken.startsWith('TEST-')) {
    console.warn(`[TenantCredentials] Access Token del tenant ${tenantId} no tiene formato esperado (APP_USR- o TEST-)`);
  }

  // Determinar environment
  const environment = (tenant.mercadoPagoEnvironment || 'sandbox') as 'sandbox' | 'production';
  ...
  console.log(`[TenantCredentials] Credenciales obtenidas para tenant ${tenantId} (environment: ${environment})`);
...
export function invalidateCredentialsCache(tenantId: string): void {
  credentialsCache.delete(tenantId);
  console.log(`[TenantCredentials] Cache invalidado para tenant ${tenantId}`);
}
...
export function clearCredentialsCache(): void {
  ...
  console.log('[TenantCredentials] Cache de credenciales limpiado completamente');
}
