...
export class BookingRepository {
  ...
  // Verificar disponibilidad (método público, fuera de transacción)
  ...
  // Verificar disponibilidad dentro de una transacción (método privado)
  private async _checkAvailabilityInTransaction(
    tx: Prisma.TransactionClient,
    data: { courtId: string; bookingDate: Date; startTime: string; endTime: string },
    excludeBookingId?: string
  ): Promise<boolean> {
    const [y, m, d] = data.bookingDate.toISOString().split('T')[0].split('-').map(Number);
  ...
  async create(data: BookingCreateInput, expirationMinutes: number = 15): Promise<BookingWithRelations> {
    // Calcular expiresAt antes de la transacción
    const expiresAt = new Date(Date.now() + expirationMinutes * 60 * 1000);

    return await this.prisma.$transaction(async (tx) => {
      // PASO 1: Verificar disponibilidad DENTRO de la transacción
      const isAvailable = await this._checkAvailabilityInTransaction(tx, {
        courtId: data.courtId,
      ...
      // PASO 2: Crear la reserva con expiresAt
      ...
      // Timeout de transacción más largo para operaciones complejas
      maxWait: 5000,
      timeout: 10000,
    });
  }
  ...
  async update(id: string, data: BookingUpdateInput): Promise<BookingWithRelations> {
    return await this.prisma.$transaction(async (tx) => {
      ...
      // También agregar closedAt si el turno ya está COMPLETED pero no tiene closedAt
      if (data.status === 'COMPLETED') {
        // Verificar si ya tiene closedAt, si no, agregarlo
        const current = await tx.booking.findUnique({ where: { id }, select: { status: true, closedAt: true } });
        if (!current?.closedAt) {
          updateData.closedAt = new Date();
        }
      } else {
        // Verificar si el turno actual está COMPLETED pero sin closedAt
        const current = await tx.booking.findUnique({ where: { id }, select: { status: true, closedAt: true } });
        if (current?.status === 'COMPLETED' && !current.closedAt) {
          updateData.closedAt = new Date();
        }
      }
