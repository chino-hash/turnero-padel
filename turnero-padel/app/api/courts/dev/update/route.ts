...
// POST /api/courts/dev/update - Actualiza una cancha y emite courts_updated (solo dev)
export async function POST(request: NextRequest) {
  try {
    ...
    eventEmitters.courtsUpdated({
      ...
      isTest: true,
      source: 'courts-dev-update'
    })

    return NextResponse.json({ success: true, court })
  } catch (error) {
    console.error('Error en POST /api/courts/dev/update:', error)
    return NextResponse.json(
