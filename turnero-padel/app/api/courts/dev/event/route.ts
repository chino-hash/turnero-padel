...
// POST /api/courts/dev/event - Emitir eventos SSE en desarrollo (prefijo permitido por middleware)
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, event: { type, message }, status: 'emitted' })
  } catch (error) {
    console.error('Error en POST /api/courts/dev/event:', error)
