...
export async function GET() {
  try {
    ...
    const now = new Date() // Para comparar con expiresAt

    // Obtener reservas, excluyendo PENDING expiradas
    // Incluir: CONFIRMED siempre, y PENDING solo si no están expiradas (expiresAt > now o null)
    const bookings = await prisma.booking.findMany({
          ...
          },
          {
            OR: [
              ...
              { 
                status: 'PENDING',
                OR: [
                  { expiresAt: null }, // Reservas antiguas sin expiresAt (compatibilidad)
                  { expiresAt: { gt: now } } // Solo PENDING no expiradas
                ]
              }
            ]
          }
      ...
      select: { 
        ...
        endTime: true, 
        status: true,
        expiresAt: true // Agregado para poder filtrar si es necesario
      },
    })
