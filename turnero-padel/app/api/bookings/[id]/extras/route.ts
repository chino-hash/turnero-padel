...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Si se paga antes, siempre se asigna a todos y no requiere playerId
    if (payload.paidBefore) {
      // Cuando paga antes, forzar assignedToAll a true
      payload.assignedToAll = true
      payload.playerId = undefined
    } else {
      // Validaciones normales cuando se paga después
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const totalPrice = unitPrice * payload.quantity // Siempre guardar el precio real
      const notes = payload.paidBefore 
        ? `${payload.notes ? payload.notes + ' - ' : ''}Pagado antes del turno`
        : payload.notes
      ...
      const created = await tx.bookingExtra.create({
        data: {
          ...
          unitPrice,
          totalPrice, // Será 0 si paidBefore es true
          assignedToAll: payload.assignedToAll || payload.paidBefore, // Si paga antes, siempre asignado a todos
          notes,
        }
      })
