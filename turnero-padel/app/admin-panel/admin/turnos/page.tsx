...
export default function TurnosPage() {
  ...
  // Calcular usuarios activos desde bookings reales
  const usuariosActivos = useMemo(() => {
    // Contar usuarios únicos con reservas activas (no canceladas, no completadas, no eliminadas)
    const usuariosConReservas = new Set<string>()
    bookings?.forEach((b: any) => {
      if (b.status !== 'CANCELLED' && b.status !== 'COMPLETED' && !b.deletedAt && b.userId) {
        usuariosConReservas.add(b.userId)
      }
    })
    return usuariosConReservas.size
  }, [bookings])
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{bookingsLoading ? '...' : usuariosActivos}</div>
            <p className="text-xs text-muted-foreground" aria-live="polite">
              {bookingsLoading ? 'Cargando' : 'Con reservas activas'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            Estado: <span className="text-green-600">🟢 Libre</span>, <span className="text-red-600">🔴 Ocupado</span>, <span className="text-yellow-600">🟡 Pendiente de pago</span>.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={showCreateBookingModal} onOpenChange={setShowCreateBookingModal}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-6">
          ...
          <div className="space-y-8 px-1">
            ...
            <div className="space-y-5 bg-muted p-5 rounded-lg">
              ...
              <div className="grid grid-cols-1 gap-5">
                <div className="space-y-2">
                  <Label htmlFor="userName" className="text-sm font-medium text-foreground">Nombre Completo *</Label>
                  <Input
                    id="userName"
            ...
            <div className="space-y-5 bg-muted p-5 rounded-lg border border-primary/20">
              <h3 className="text-lg font-semibold flex items-center gap-2 text-foreground">
                <FileText className="w-5 h-5" />
                Turno Fijo (Opcional)
              </h3>
              <p className="text-xs text-muted-foreground">Si seleccionas esta opción, se creará una regla recurrente que bloqueará este horario cada semana hasta que se dé de baja.</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-foreground">Activar turno fijo</Label>
            ...
            <div className={`space-y-5 bg-muted p-5 rounded-lg border border-primary/20 ${isRecurring ? 'opacity-60' : ''}`} aria-disabled={isRecurring}>
              <h3 className="text-lg font-semibold flex items-center gap-2 text-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="space-y-2">
                  <Label htmlFor="courtName" className="text-sm font-medium text-foreground">Cancha *</Label>
                ...
                <div className="space-y-2">
                  <Label htmlFor="timeRange" className="text-sm font-medium text-foreground">Horario *</Label>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        ref={timeTriggerRef}
            ...
            <div className="flex justify-end gap-4 pt-6 mt-6 border-t border-border">
              ...
              {isRecurring && (
                <Button
                  ...
                  className="px-6 py-2.5 h-auto text-primary border-primary/50 hover:bg-accent"
