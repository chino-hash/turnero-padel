...
const AdminTurnos: React.FC<AdminTurnosProps> = ({ className = "", isDarkMode: propIsDarkMode }) => {
  ...
  const [paidBefore, setPaidBefore] = useState<boolean>(false)
  ...
  // Validación para completar turno
  ...
  const getCategoryAndRemaining = (booking: Booking) => {
    ...
    if (booking.status === 'completado') {
      if (booking.closedAt) {
        console.log('Turno completado con closedAt:', booking.id, booking.closedAt, '-> category: closed')
        return { category: 'closed' as const, remainingMs: 0 }
      }
      console.log('Turno completado sin closedAt:', booking.id, '-> category: completed')
    ...
    // En curso: desde que empieza hasta que el admin lo termina manualmente
    // Sigue en 'in_progress' incluso después de que termine el tiempo (timer negativo)
    if (now >= start) {
      const remainingMs = end.getTime() - now.getTime()
      return { category: 'in_progress' as const, remainingMs }
    }
      ...
      if (booking && booking.id) {
        setBookings(prev => prev.map(b => {
          ...
          const mappedExtras = (booking.extras || [])
            ...
            .map((e: any) => {
              // Detectar si fue pagado antes (notas contienen "Pagado antes")
              const isPaidBefore = e.notes?.includes('Pagado antes')
                return {
                  id: e.id,
                  type: 'otro',
                  name: isPaidBefore ? `${e.producto?.nombre || 'Extra'} (Pagado antes)` : (e.producto?.nombre || 'Extra'),
                  cost: e.totalPrice, // Precio real (no 0)
                  assignedTo: e.assignedToAll ? 'all' : (e.player?.position ? `player${e.player.position}` : 'player1'),
                  notes: e.notes // Incluir notas para detectar extras pagados antes
                }
            })
  ...
  const openExtrasModal = (bookingId: string) => {
    ...
    setPaidBefore(false)
    setSelectedPlayers(["player1","player2","player3","player4"])
  }
  ...
  const handleAddExtra = async () => {
    ...
    if (!paidBefore && selectedPlayers.length === 0) return
    setAddingExtra(true)
    ...
    try {
      // Si se paga antes, no se divide ni se suma al importe final
      if (paidBefore) {
        // Solo registrar el extra como pagado antes, sin asignarlo a jugadores ni sumarlo al total
        const postWithRetry = async (retries: number, delayMs: number): Promise<Response> => {
          const res = await timedFetch(`/api/bookings/${selectedBookingId}/extras`, {
            ...
            body: JSON.stringify({
              ...
              quantity,
              assignedToAll: true,
              paidBefore: true, // Flag para indicar que ya fue pagado
            })
        ...
        if (!res.ok) {
          // Fallback: agregar como extra local con precio real pero marcado como pagado antes
          const total = producto.precio * quantity
          addExtra(selectedBookingId, {
            type: 'otro',
            name: `${producto.nombre} (Pagado antes)`,
            cost: total, // Precio real para mostrarlo
            assignedTo: 'all'
          })
      ...
      // Lógica original para cuando se paga después
  ...
  }, [bookings, searchTermDebounced, statusFilter, dateFilter, now])
  ...
  // Terminar turno en curso - cambia de 'confirmado' a 'completado'
  const endTurno = async (bookingId: string) => {
    const current = bookings.find(b => b.id === bookingId)
    if (!current) return
    
    // Solo se pueden terminar turnos que estén en curso (status confirmado y categoría in_progress)
    const cat = getCategoryAndRemaining(current).category
    if (cat !== 'in_progress' || current.status !== 'confirmado') {
      console.warn('No se puede terminar un turno que no está en curso')
      return
    }

    const previousStatus = current.status
    // Actualización optimista
    setBookings(prev => prev.map(b => (b.id === bookingId ? { ...b, status: 'completado' as const } : b)))
    ...
    try {
      ...
      if (!res.ok) {
        // Revertir si falla
        setBookings(prev => prev.map(b => (b.id === bookingId ? { ...b, status: previousStatus } : b)))
        console.error('Error al terminar turno:', res.status)
      }
    } catch (err) {
      // Revertir si falla
      setBookings(prev => prev.map(b => (b.id === bookingId ? { ...b, status: previousStatus } : b)))
  ...
  const closeBooking = async (bookingId: string) => {
    ...
    // Separar extras pagados antes de los que se pagan después
    const extrasPaidBefore = current.extras.filter(e => {
      return e.notes?.includes('Pagado antes') || e.name?.includes('(Pagado antes)')
    })
    const extrasPaidAfter = current.extras.filter(e => {
      return !e.notes?.includes('Pagado antes') && !e.name?.includes('(Pagado antes)')
    })
    
    // Calcular el total esperado de todos los jugadores
    const keys: Array<keyof Booking['players']> = ['player1', 'player2', 'player3', 'player4']
    const totalExpectedFromPlayers = keys.reduce((sum, key) => {
      return sum + calculatePlayerAmount(current, key)
    }, 0)
    
    // Total original = lo esperado de jugadores + extras que se pagan después
    const totalExtras = extrasPaidAfter.reduce((sum, extra) => sum + (extra.cost || 0), 0)
    const totalOriginal = totalExpectedFromPlayers + totalExtras
    
    // Saldo pendiente = total esperado - lo pagado por jugadores
    const playersPaid = computeAmountPaid(current)
    const pendingBalance = Math.max(0, totalOriginal - playersPaid)
    if (pendingBalance > 0) {
    ...
    try {
      ...
      if (!res.ok) {
        // Obtener el mensaje de error para debug
        let errorMessage = `Error ${res.status}`
        try {
          const errorData = await res.json()
          errorMessage = errorData?.error || errorMessage
          console.error('Error al cerrar turno:', errorMessage)
        } catch (_) {
          console.error('Error al cerrar turno: respuesta no válida')
        }
        // Mostrar error al usuario (opcional)
        alert(`No se pudo cerrar el turno: ${errorMessage}`)
        return
      }
      const data = await res.json()
      // Usar closedAt del backend si está disponible, sino crear uno nuevo
      const closedAt = data?.data?.closedAt || data?.data?.booking?.closedAt || data?.closedAt || new Date().toISOString()
      console.log('Turno cerrado exitosamente, closedAt:', closedAt, 'data completa:', data)
      // Actualizar estado local a completado y marcar closedAt para que se mueva a la sección de cerrados
      // El useEffect detectará el cambio y actualizará filteredBookings automáticamente
      setBookings(prev => {
        const updated = prev.map(b => {
          if (b.id === bookingId) {
            // Crear un nuevo objeto para asegurar que React detecte el cambio
            const newBooking = { ...b, status: 'completado' as const, closedAt: closedAt }
            console.log('Actualizando turno:', bookingId, 'closedAt:', closedAt, 'nuevo estado:', newBooking)
            return newBooking
          }
          return b
        })
  ...
  const togglePlayerPayment = (bookingId: string, playerKey: keyof Booking['individualPayments']) => {
    ...
    // Actualización simplificada: solo actualizar estado de pago (no se maneja dinero real)
    // Ejecutar en segundo plano sin bloquear la UI - respuesta inmediata
    fetch(`/api/bookings/${bookingId}`, {
      ...
      .then(putRes => {
        if (!putRes.ok && putRes.status !== 429) {
          // Solo revertir UI si hay un error crítico (no rate limit)
          console.error('Error actualizando estado de pago:', putRes.status)
          if (previousState) {
            setBookings(prevBookings => prevBookings.map(b => (b.id === bookingId ? previousState : b)))
          }
        }
      ...
      .catch(err => {
        console.error('Error en actualización de pago:', err)
        // Revertir UI optimista en caso de error de red
        if (previousState) {
          setBookings(prevBookings => prevBookings.map(b => (b.id === bookingId ? previousState : b)))
        }
      })
  ...
  const renderExpandedContent = (booking: Booking, idx: number) => {
    ...
    // Separar extras pagados antes de los que se pagan después
    const extrasPaidBefore = booking.extras.filter(e => {
      // Detectar si fue pagado antes: las notas contienen "Pagado antes" o el nombre contiene "(Pagado antes)"
      return e.notes?.includes('Pagado antes') || e.name?.includes('(Pagado antes)')
    })
    const extrasPaidAfter = booking.extras.filter(e => {
      return !e.notes?.includes('Pagado antes') && !e.name?.includes('(Pagado antes)')
    })
    
    // Total de extras que se suman al importe original (solo los que se pagan después)
    const totalExtras = extrasPaidAfter.reduce((sum, extra) => sum + (extra.cost || 0), 0)
    // Total de extras pagados antes (se suman a lo pagado pero no al total original)
    const totalExtrasPaidBefore = extrasPaidBefore.reduce((sum, extra) => sum + (extra.cost || 0), 0)
    
    // Calcular el total esperado de todos los jugadores
    // Solo incluir extras que se pagan después (no los pagados antes)
    const keys: Array<keyof Booking['players']> = ['player1', 'player2', 'player3', 'player4']
    
    // Función auxiliar para calcular monto de jugador sin extras pagados antes
    const calculatePlayerAmountWithoutPaidBefore = (playerKey: keyof Booking['players']): number => {
      const baseAmount = booking.totalPrice / 4
      const playerExtras = extrasPaidAfter.filter(
        extra => extra.assignedTo === playerKey || extra.assignedTo === 'all'
      )
      const extrasAmount = playerExtras.reduce((sum, extra) => {
        if (extra.assignedTo === 'all') {
          return sum + extra.cost / 4
        }
        return sum + extra.cost
      }, 0)
      return baseAmount + extrasAmount
    ...
    // Total esperado = suma de lo que debe pagar cada jugador (solo extras que se pagan después)
    const totalExpectedFromPlayers = keys.reduce((sum, key) => {
      return sum + calculatePlayerAmountWithoutPaidBefore(key)
    }, 0)
    
    // Total original = lo esperado de jugadores (ya incluye extras que se pagan después)
    const totalOriginal = totalExpectedFromPlayers
    
    // Monto pagado (incluye extras pagados antes para mostrar en la tarjeta)
    const amountPaid = computeAmountPaid(booking) + totalExtrasPaidBefore
    
    // Calcular cuánto pagaron los jugadores SOLO por extras que se pagan después
    // (excluyendo extras pagados antes del cálculo del saldo pendiente)
    const playersPaidForPendingExtras = keys.reduce((sum, key) => {
      const status = booking.individualPayments[key as keyof Booking['individualPayments']]
      if (status !== 'pagado') return sum
      return sum + calculatePlayerAmountWithoutPaidBefore(key)
    }, 0)
    
    // Saldo pendiente = total esperado - lo pagado por jugadores (solo extras que se pagan después)
    // Esto asegura que si hay jugadores sin pagar, el saldo será positivo
    const pendingBalance = Math.max(0, totalOriginal - playersPaidForPendingExtras)
    ...
    const disableToggle = (isClosed && isPaid && pendingBalance === 0)
        ...
        <div className="flex gap-2 pt-2">
          ...
          {(() => {
            // Botón "Terminar turno" solo para turnos en curso
            const cat = getCategoryAndRemaining(booking).category
            const isInProgress = cat === 'in_progress' && booking.status === 'confirmado'
            if (isInProgress) {
              return (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => endTurno(booking.id)}
                  className="text-orange-600 border-orange-600 hover:bg-orange-50"
                  data-testid={`admin-end-turno-btn-${idx + 1}`}
                >
          ...
          {(() => {
            // Solo se pueden cerrar turnos completados (no confirmados)
            // Los turnos confirmados aún no se jugaron, no se pueden cerrar
            const canClose = booking.status === 'completado' && !booking.closedAt && pendingBalance === 0
            const isClosed = booking.status === 'completado' && !!booking.closedAt
            return (
              <Button
                variant="outline"
            ...
            <div>
              ...
              <div className="mt-4 space-y-4">
                {fixedDerived.slice(0, visibleFixed).map((d, idx) => (
                    <Card key={d.booking.id} className="overflow-hidden py-2 gap-2">
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <CardTitle className="text-base flex items-center gap-2">
                              ...
                              {(() => {
                                ...
                                return (
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${BOOKING_STATUS_COLORS[statusKey]}`}>
                                    {BOOKING_STATUS_LABELS[statusKey]}
                                  </span>
                                )
                              })()}
                              <span className="px-2 py-0.5 rounded-full text-xs font-medium border bg-purple-100 text-purple-800 border-purple-200">Fijo</span>
                            </CardTitle>
                          ...
                          <div className="flex items-center gap-2">
                            {(() => {
                              ...
                              return (
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getPaymentStatusColor(d.booking.paymentStatus, isDarkMode)}`}>
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex items-center gap-2">
                            {(() => {
                              ...
                              if (cat === 'completed') {
                                return (
                                  <span className="px-2 py-0.5 rounded-full text-xs font-medium border bg-yellow-100 text-yellow-800 border-yellow-200">Finalizada · confirmar cierre</span>
                                )
                              }
                              return (
                                <span className="px-2 py-0.5 rounded-full text-xs font-medium border bg-gray-100 text-gray-800 border-gray-200">Completado</span>
                              )
                            })()}
                            {(() => {
                              const formatCurrency = (value: number) => new Intl.NumberFormat('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value)
                        ...
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <CardTitle className="text-base flex items-center gap-2">
                              <MapPin className="w-4 h-4 text-blue-600" />
                              {d.booking.courtName}
                              <span className="px-2 py-0.5 rounded-full text-xs font-medium border bg-gray-100 text-gray-800 border-gray-200">Cerrado</span>
                            </CardTitle>
                            <div className="flex items-center gap-4 text-sm text-white mt-1">
                              <span className="flex items-center gap-1">
                                <Calendar className="w-4 h-4" />
                          ...
                          <div className="flex items-center gap-2">
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium border bg-gray-100 text-gray-800 border-gray-200">Sin acciones</span>
  ...
  {showExtrasModal && (
          ...
          <div className={`rounded-lg p-6 w-full max-w-md mx-4 max-h-[80vh] overflow-y-auto ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}>
            ...
            <div className="space-y-4">
              <div className="relative product-search-container">
                ...
                {showProductResults && filteredProductos.length > 0 && (
                    ...
                    {filteredProductos.map((p) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 ${
                          selectedProductId === p.id ? 'bg-blue-100 dark:bg-blue-900' : ''
                        } ${
                          !p.activo || p.stock <= 0 ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium">{p.nombre}</span>
                          <div className="flex items-center gap-2 text-xs">
                            <span className="text-gray-500">${p.precio.toLocaleString()}</span>
                            {!p.activo && <span className="text-red-500">INACTIVO</span>}
                            {p.stock <= 0 && <span className="text-red-500">SIN STOCK</span>}
                          </div>
                        </div>
                        {p.stock > 0 && p.activo && (
                ...
                {showProductResults && productSearchTerm.trim() && filteredProductos.length === 0 && (
                  <div className={`absolute z-10 w-full mt-1 border rounded-md shadow-lg p-4 ${
                    isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
                  }`}>
                ...
                <div className="mt-1 text-xs">
                  {selectedProduct ? (
                    selectedProduct.activo && selectedProduct.stock > 0 ? (
                      <span className="text-green-600">Disponible · Stock: {selectedProduct.stock} · Precio: ${selectedProduct.precio.toLocaleString()}</span>
                    ) : (
                      <span className="text-red-500">No disponible · { !selectedProduct.activo ? 'Inactivo' : 'Sin stock' }</span>
                    )
                  ) : (
                    <span className="text-gray-500">Escribe el nombre del producto para buscarlo</span>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Button
                    type="button"
                    variant={paidBefore ? "default" : "outline"}
                    className="flex-1"
                    onClick={() => setPaidBefore(true)}
                  >
                    Paga Antes
                  </Button>
                  <Button
                ...
                {paidBefore && (
                  <p className="text-xs text-muted-foreground mt-1">
                    El extra ya fue pagado. No se dividirá entre jugadores ni se sumará al importe final.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <Popover>
                  <PopoverTrigger 
                    className="flex h-10 items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm w-full"
                    disabled={paidBefore}
                  >
                    <span className="line-clamp-1">
                      {paidBefore 
                        ? 'No aplica (paga antes)'
            ...
            <div className="flex gap-2 mt-6">
              ...
              <Button 
                ...
                disabled={
                  ...
                  (!paidBefore && selectedPlayers.length === 0)
                }
              >
      ...
      {/* Modal de confirmación para completar turno */}
      {confirmBookingId && (
        <Dialog open={true} onOpenChange={(open) => { if (!open) { setConfirmBookingId(null) } }}>
          <DialogContent>
            <DialogHeader>
            ...
            <DialogFooter className="mt-4">
              <Button variant="outline" onClick={() => { setConfirmBookingId(null) }}>Cancelar</Button>
              <Button 
                onClick={() => { 
                  if (confirmBookingId) { 
                    closeBooking(confirmBookingId)
