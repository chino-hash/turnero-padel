...
function PadelBookingPage() {
  ...
  const createBookingApi = async (slot: any) => {
    // Obtener courtId del slot o usar el seleccionado
    const courtId = slot?.courtId || selectedCourt
    if (!courtId) {
      throw new Error('No se ha seleccionado una cancha')
    }

    // Obtener startTime y endTime del slot
    let startTime = slot.startTime
    let endTime = slot.endTime

    // Si no están disponibles directamente, intentar extraerlos de timeRange
    if (!startTime || !endTime) {
      if (slot.timeRange) {
        ...
        // Si solo hay time, calcular endTime (asumiendo 90 minutos de duración)
        startTime = slot.time
        const [hours, minutes] = startTime.split(':').map(Number)
        const endDate = new Date()
        endDate.setHours(hours, minutes + 90, 0, 0)
        endTime = `${String(endDate.getHours()).padStart(2, '0')}:${String(endDate.getMinutes()).padStart(2, '0')}`
      }
    }

    if (!startTime || !endTime) {
      throw new Error('No se pudo determinar el horario del turno')
    }

    // Usar la fecha del slot si está disponible, de lo contrario usar selectedDate
    let bookingDate = selectedDate
    if (slot?.date) {
      bookingDate = slot.date instanceof Date ? slot.date : new Date(slot.date)
    }
  ...
  const handleConfirmFromSlotModal = async (slot: any) => {
    try {
      ...
      console.error("Error creando reserva:", e)
      throw e // Re-lanzar el error para que SlotModal lo maneje
    }
  }
    ...
    <div className="dashboard-theme font-sans">
      ...
      <div className="pt-14 sm:pt-16">
        ...
        <HomeSection
          ...
          error={slotsError}
          onRetry={retrySlots}
          onConfirmReservation={handleConfirmFromSlotModal}
        />
